...
#define LM87_REG_VCCP1                   0x21  /* this will be CPU core voltage */
...
static ctl_table LM87_dir_table_template[] = {
	{LM87_SYSCTL_IN0, "2.5V", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_in},
	{LM87_SYSCTL_IN1, "cpu_volt", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_in},
	{LM87_SYSCTL_IN2, "3.3V", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_in},
	{LM87_SYSCTL_IN3, "5V", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_in},
	{LM87_SYSCTL_IN4, "12V", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_in},
	{LM87_SYSCTL_IN5, "Vccp2", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_in},
	{LM87_SYSCTL_FAN, "fan", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_fan},
        {LM87_SYSCTL_FRNT_TEMP, "front_amb_temp", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_temp},
	{LM87_SYSCTL_CPU_TEMP, "cpu_temp", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_temp},
	{LM87_SYSCTL_INT_TEMP, "int_temp", NULL, 0, 0644, NULL, &sensors_proc_real,
	  &sensors_sysctl_real, NULL, &LM87_temp},
...
void LM87_update_client(struct i2c_client *client)
{
	...
	if ((jiffies - data->last_updated > HZ) ||  /* 1 sec cache */
		...
		for (i = 0; i <= 5; i++) {  
		     ...
		     * accomodate with a hardcode register address and we only get
		  ...
		  int *nrels_mag, long *results)
{
	...
	else if (operation == SENSORS_PROC_REAL_READ) 
	{
	   ...
	   *nrels_mag = 3;
		...
		if (*nrels_mag >= 1) {
	           if (ctl_name == LM87_SYSCTL_FRNT_TEMP) {
			data->front_amb_temp_max = TEMP_LIMIT_TO_REG(results[0]);
			LM87_write_value(client, LM87_REG_2_5V_EXT_TEMP_2_HIGH,
					    data->front_amb_temp_max);
		   }
		...
		if (*nrels_mag >= 2) {
	           if (ctl_name == LM87_SYSCTL_FRNT_TEMP) {
			data->front_amb_temp_min = TEMP_LIMIT_TO_REG(results[0]);
			LM87_write_value(client, LM87_REG_2_5V_EXT_TEMP_2_LOW,
					    data->front_amb_temp_min);
		   }
		   if (ctl_name == LM87_SYSCTL_CPU_TEMP) {
