...
sensor values. This program is called `sensors`. You can use it as a
...
This package does not contain a nice graphical monitor. See the following projects:
...
almost every time the problem will be elsewhere. The most likely culprit
for a problem is one of the kernel drivers providing data to lm-sensors
or, rather, the `sensors` command (and libsensors). If you encounter problems,
the following tests are recommended.

1. Check enabled sensors with the `sensors` command. Observe and, if there
   are errors, write down your test steps and report.
2. If any problems are observed while running the `sensors` command, disable
   ...
   maintainer.
3. If problems are observed while using libsensors from another application,
...
4. If this does not help, try to run `strace` on the `sensors` command and/or
...
- **Mailing list:** lm-sensors@vger.kernel.org
  ([subscription info and archives](http://vger.kernel.org/vger-lists.html#lm-sensors);
...
License (LGPL), as included in the file [COPYING.LGPL](COPYING.LGPL). The rest of this
package may be distributed according to the GNU General Public License
(GPL), as included in the file [COPYING](COPYING).
